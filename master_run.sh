#bash runner_poison_30p.sh square cifar10 ours
#sleep 2
...
#bash runner_poison_30p.sh plus_distributed cifar10 ours
#sleep 2
#pkill -u "$(whoami)" pt_main_thread


#bash runner_poison_30p.sh square fmnist ours
#sleep 2
#pkill -u "$(whoami)" pt_main_thread
...
bash runner_poison_30p.sh plus fmnist ours
sleep 2
pkill -u "$(whoami)" pt_main_thread
#python generate_final_round_table.py "$(ls -t ./results/fmnist/*.txt | head -n 1)"
#python generate_all_rounds_table.py "$(ls -t ./results/fmnist/*.txt | head -n 1)"
...
#commented out test below because code for plus_distributed poisoning pattern is still being fixed/updated
#bash runner_poison_30p.sh plus_distributed fmnist ours
#sleep 2
#pkill -u "$(whoami)" pt_main_thread

bash runner_poison_30p.sh plus cifar10 rlr
sleep 2
pkill -u "$(whoami)" pt_main_thread

bash runner_poison_30p.sh plus cifar10 nodefense
sleep 2
pkill -u "$(whoami)" pt_main_thread

bash runner_poison_30p.sh plus fmnist rlr
sleep 2
pkill -u "$(whoami)" pt_main_thread

bash runner_poison_30p.sh plus fmnist nodefense
sleep 2
pkill -u "$(whoami)" pt_main_thread


bash runner_poison_30p.sh plus fmnist ours_hybrid
sleep 2
pkill -u "$(whoami)" pt_main_thread

bash runner_poison_30p.sh plus cifar10 ours_hybrid
sleep 2
pkill -u "$(whoami)" pt_main_thread
