#!/bin/bash
# with 30% attack ratio: 30% of 33 clients = 10 are malicious
# server.py is modified to support 30% attack ratio using fedmnist
set -e
...
for (( i=0; i<10; i++ )); do
    ...
    python client.py \
        --poison POISON \  # this option is irrelevant for non iid code logic
        --pattern "$pattern" \
        --clientID "$clientID" \
        --data "$data" \
        --use_cuda True \
...
echo "--- Starting 4 Normal Clients on ${NORMAL_GPU_1} ---"
for (( i=10; i<14; i++ )); do
    clientID=$i
